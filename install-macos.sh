...
install_official_if_missing(){
  if ! command -v tailscale >/dev/null 2>&1; then
    ...
    if command -v brew >/dev/null 2>&1; then
      ...
      err "Homebrew not found. Install from https://tailscale.com/download and rerun."; exit 1
    fi
  else
...
install_binaries(){
  if [[ "$VERSION" == "latest" ]]; then
    ...
    [[ -z "$tag" ]] && { err "Failed to resolve latest release"; exit 1; }
    VERSION="$tag"; info "Latest version: $VERSION"
  fi

  local base_url="https://github.com/$REPO/releases/download/$VERSION"
  local ts="tailscale-$platform"; local tsd="tailscaled-$platform"
  local tmp
  tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT; pushd "$tmp" >/dev/null
  info "Downloading $ts"; curl -fL "$base_url/$ts" -o tailscale
  info "Downloading $tsd"; curl -fL "$base_url/$tsd" -o tailscaled
  ...
  info "Installing to $TS_PATH"; $SUDO install -m 0755 tailscale "$TS_PATH"
  info "Installing to $TSD_PATH"; $SUDO install -m 0755 tailscaled "$TSD_PATH"

  popd >/dev/null; rm -rf "$tmp"; trap - EXIT; ok "Binaries installed"
}

usage(){
  echo ""; echo "Quick Start:"; echo "  tailscale up"; echo "  tailscale amnezia-wg set"; echo "  tailscale amnezia-wg get"; echo "  tailscale amnezia-wg reset"; echo ""
}
